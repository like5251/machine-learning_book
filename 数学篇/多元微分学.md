...
矩阵求导的技术，在统计学、控制论、机器学习等领域有广泛的应用。鉴于我看过的一些资料或言之不详、或繁乱无绪，本文来做个科普。
...
思路：因为全微分可以表示为矩阵导数和$$dX$$的内积，而$$df$$又是标量，则有

$$
df = \text{tr}(df) = \text{tr}\left(f^{'}(X)^T dX\right)
$$

因此我们可以通过微分化简和迹化简两步得到全导数：
1. 微分化简：通过微分化简提出$$dX$$
2. 迹化简：通过迹化简将$$dX$$移至微分右侧，则微分中$$dX$$左侧部分既是导数的转置
...
记忆：微分运算法则总体上是将括号内微分对象移至括号外的规则，对于大多数微分运算我们都可以通过在脑海中联想$$dX$$的定义，将括号内运算作用于每个元素之后再写出结果的矩阵形式得到，但是对于乘法、逆、行列式最好直接记住它的推导过程和结果！
#### 矩阵微分复合法则
对符合函数求导$$f^{'}(U(X))$$，其中$$U$$和$$X$$均为矩阵，有以下微分复合法则：

$$
\begin{align*}
df=tr(df)&=tr(f^{'}(U)^TdU)\\&=tr(f^{'}(U)^Ttr(U^{'}(X)^TdX))
\end{align*}
$$
...
- 例5【方差的最大似然估计】：样本$$\boldsymbol{x}_1,\dots, \boldsymbol{x}_n\sim N(\boldsymbol{\mu}, \Sigma)$$，其中$$\Sigma$$是对称正定矩阵，求方差$$\Sigma$$的最大似然估计。写成数学式是：$$l = \log|\Sigma|+\frac{1}{n}\sum_{i=1}^n(\boldsymbol{x}_i-\boldsymbol{\bar{x}})^T\Sigma^{-1}(\boldsymbol{x}_i-\boldsymbol{\bar{x}})$$，求$$\frac{\partial l }{\partial \Sigma}$$的零点
